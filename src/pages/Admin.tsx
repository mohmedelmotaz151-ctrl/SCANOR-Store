...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'customers' | 'wallet' | 'reports' | 'support'>('orders');
  ...
  useEffect(() => {
    ...
    const qChats = query(collection(db, "support_chats"), orderBy("lastMessageAt", "desc"));
    const unsubChats = onSnapshot(qChats, 
      (snapshot) => {
  ...
  useEffect(() => {
    ...
    if (selectedChat) {
      ...
      unsubMsgs = onSnapshot(qMsgs, (snapshot) => {
        ...
        setSelectedChat((prev: any) => ({ ...prev, messages: msgs }));
      }, (error) => handleFirestoreError(error, OperationType.LIST, `support_chats/${selectedChat.id}/messages`));
    }
    return () => unsubMsgs?.();
  }, [selectedChat?.id]);
  ...
  const sendAdminReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const messagesRef = collection(db, "support_chats", selectedChat.id, "messages");
      ...
      <div className="flex flex-wrap items-center justify-between gap-6 mb-12">
        ...
        <div className="flex gap-2 bg-neutral-900 p-1 rounded-2xl border border-neutral-800">
          ...
          <button 
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'support' ? 'bg-amber-500 text-black' : 'text-neutral-500 hover:text-white'}`}
          >
            الدعم
          </button>
        </div>
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
        <div className="p-8 border-b border-neutral-800 bg-neutral-900/50 flex justify-between items-center">
          <h3 className="font-bold text-xl uppercase tracking-tight text-right w-full text-white" dir="rtl">
            {activeTab === 'orders' ? 'الطلبات الأخيرة' : activeTab === 'wallet' ? 'طلبات شحن المحفظة' : activeTab === 'customers' ? 'قائمة العملاء' : activeTab === 'support' ? 'محادثات الدعم الفني' : 'تقارير المبيعات'}
          </h3>
        </div>
        ...
        <div className="overflow-x-auto min-h-[400px]">
          {activeTab === 'orders' ? (
            ...
            <div className="flex h-[600px]" dir="rtl">
              {/* Chat List */}
              <div className="w-1/3 border-l border-neutral-800 overflow-y-auto">
                {chats.length === 0 && (
                  <div className="p-8 text-center text-neutral-500 text-sm">لا توجد محادثات نشطة</div>
                )}
                {chats.map(chat => (
                  <button 
                    key={chat.id}
                    onClick={() => setSelectedChat(chat)}
                    className={`w-full p-6 text-right border-b border-neutral-800 transition-colors flex flex-col gap-1 ${selectedChat?.id === chat.id ? 'bg-amber-500/10 border-r-4 border-r-amber-500' : 'hover:bg-neutral-800/50'}`}
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-white text-sm">{chat.userEmail}</span>
                      <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${chat.status === 'active' ? 'bg-blue-500/10 text-blue-500' : chat.status === 'waiting' ? 'bg-amber-500/20 text-amber-500 animate-pulse' : 'bg-neutral-800 text-neutral-500'}`}>
                        {chat.status}
                      </span>
                    </div>
                    <span className="text-[10px] text-neutral-500">{new Date(chat.lastMessageAt?.toDate?.() || chat.lastMessageAt).toLocaleString()}</span>
                  </button>
                ))}
              </div>
                ...
                {selectedChat ? (
                  <>
                    <div className="p-4 border-b border-neutral-800 flex justify-between items-center bg-neutral-900/50">
                       <span className="font-bold text-sm text-neutral-400">{selectedChat.userEmail}</span>
                       <div className="flex items-center gap-2">
                         <span className="text-[10px] text-neutral-500">ID: {selectedChat.id}</span>
                       </div>
                    </div>
                    <div className="flex-1 overflow-y-auto p-6 space-y-4">
                      {selectedChat.messages?.map((msg: any) => (
                        <div key={msg.id} className={`flex ${msg.role === 'agent' ? 'justify-start' : 'justify-end'}`}>
                          <div className={`max-w-[80%] p-3 rounded-2xl text-xs leading-relaxed ${msg.role === 'agent' ? 'bg-amber-500 text-black font-bold' : 'bg-neutral-800 text-neutral-300 border border-neutral-700'}`}>
                            {msg.text}
                          </div>
                        </div>
                      ))}
                    </div>
                    <form onSubmit={sendAdminReply} className="p-4 bg-neutral-950 border-t border-neutral-800 flex gap-2">
                      <input 
                        type="text" 
                        placeholder="اكتب ردك هنا..."
                        className="flex-1 bg-neutral-800 border-none rounded-xl px-4 py-3 text-sm focus:ring-1 focus:ring-amber-500"
                        value={adminReply}
                        onChange={(e) => setAdminReply(e.target.value)}
                      />
                      <button className="px-6 py-2 bg-amber-500 text-black rounded-xl font-bold text-sm hover:bg-amber-400 transition-all">إرسال</button>
                    </form>
                  </>
                ) : (

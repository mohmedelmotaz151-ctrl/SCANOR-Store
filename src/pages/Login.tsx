...
export default function Login() {
  ...
  const [confirmationResultSent, setConfirmationResultSent] = useState(false);
  ...
  // Handle reCAPTCHA initialization for React correctly as requested
  useEffect(() => {
    ...
    return () => {
      if (window.recaptchaVerifier) {
        // window.recaptchaVerifier.clear(); // Optional: clean up if needed
      }
    };
  }, [method]);
  ...
  const handlePhoneAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (!confirmationResultSent) {
        await sendOTP(phoneNumber);
        setConfirmationResultSent(true);
        setSuccess(t('otp_success_msg'));
      } else {
        const user = await verifyOTP(verificationCode);
        if (user) {
          await syncUserToFirestore(user);
          navigate("/");
        }
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <div className="flex bg-neutral-950 p-1.5 rounded-2xl mb-8 border border-neutral-800">
              <button 
                onClick={() => { setMethod("email"); setError(""); setConfirmationResultSent(false); }}
              ...
              <button 
                onClick={() => { setMethod("phone"); setError(""); setConfirmationResultSent(false); }}
              ...
              <form onSubmit={handlePhoneAuth} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      required
                      value={phoneNumber}
                      disabled={confirmationResultSent}
                      onChange={(e) => setPhoneNumber(e.target.value)}
                ...
                >
                  {loading ? (
                    ...
                    <>
                      {!confirmationResultSent ? t('send_code_btn') : t('verify_btn')}
                      <ChevronRight className={`w-5 h-5 ${language === 'ar' ? 'rotate-180' : ''}`} />
                    </>
                  )}
                </button>
...
// Global type declarations shifted to authService.ts

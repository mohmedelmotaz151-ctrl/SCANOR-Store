...
export default function Home() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const pkgsCollection = collection(db, "packages");
        const snapshot = await getDocs(query(pkgsCollection, orderBy("amount", "asc")));
        
        if (snapshot.empty) {
          // SEED DATA if empty
          const initialPackages = [
            { amount: 60, price_sar: 4.1, price_sdg: 4100, bonus: 0, image: "" },
            { amount: 325, price_sar: 19.29, price_sdg: 20833, bonus: 25, image: "" },
            { amount: 660, price_sar: 36.49, price_sdg: 39409, bonus: 60, image: "" },
            { amount: 1800, price_sar: 88.39, price_sdg: 95461, bonus: 180, image: "" },
            { amount: 3850, price_sar: 174.89, price_sdg: 188881, bonus: 450, image: "" },
            { amount: 8100, price_sar: 347.79, price_sdg: 375613, bonus: 900, image: "" }
          ];
          
          // Note: This will only work if the user is an admin or if we allow creation temporarily or if we use getFromServer carefully.
          // Since it's the home page, it might fail for non-admins if rules are strict.
          // However, for the demo we'll try to seed.
          const seeded = [];
          for (const pkg of initialPackages) {
            // We'll only seed if we can (likely only for the admin who first visits)
            try {
               const docRef = await addDoc(pkgsCollection, pkg);
               seeded.push({ id: docRef.id, ...pkg });
            } catch (e) {
               console.warn("Failed to seed package:", pkg.amount);
            }
          }
          if (seeded.length > 0) setPackages(seeded as UCPackage[]);
          else setPackages(initialPackages.map((p, i) => ({ id: `id-${i}`, ...p }))); // fallback
        } else {
          setPackages(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as UCPackage)));
        }
      } catch (err) {
        console.error("Firestore Packages fetch error:", err);
        // Fallback static packages if Firestore fails
        setPackages([
